...
SUB_AREA_TURNS = {
    "ELIG": [
        ("Inclusion criteria",
         "Read Section 4.1 (Inclusion Criteria) of the PDF. Extract a KRI for EVERY inclusion criterion AND every sub-criterion (e.g. a criterion with sub-parts a, b, c; a sub-part with items i-v). Use prefix ELIG-INC-NNN. If a criterion relies purely on investigator judgment (phrases like 'in the investigator's opinion'), use NDEF-ELIG-NNN prefix instead."),
        ("Exclusion criteria",
        ...
        ("NDEF review",
         "Review all criteria. For any criterion that is framed entirely as 'in the investigator's judgment' or 'in the opinion of the investigator' and cannot be verified by a binary rule, output it as an NDEF KRI with rule_for_llm: 'NDEF — Non-verifiable: [reason]'. Output only NEW NDEF KRIs not already produced."),
    ],
    "SAF": [
        ("AE/SAE reporting",
...
def run_gemini_extraction_multi_turn(
    ...
    Args:
        domain: "ELIG", "SAF", "END", or "OPS" (NOT "SOA" — SOA uses 6-step process)
