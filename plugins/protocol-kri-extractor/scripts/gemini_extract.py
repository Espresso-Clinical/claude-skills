...
SUB_AREA_TURNS = {
    ...
    "SAF": [
        ...
        ("Stopping rules & IP discontinuation",
         "Read Section 9 (Subject Withdrawal and Stopping). Extract every KRI about: IP discontinuation triggers (related SAE, Grade 3+ AE, Grade 3 lab, hypersensitivity, pregnancy), study pause triggers, DMC review. Use prefix SAF-STOP. Emit ONE KRI PER objective discontinuation/withdrawal trigger — a multi-trigger list is never a single rule; a subjective trigger ('at investigator discretion') is captured faithfully with its framing, not bundled. Output only NEW KRIs."),
        ("Solicited AEs & infusion monitoring",
         "Read the Solicited AEs section (typically Section 6.2x). Extract every KRI about: each solicited AE listed (e.g. malaise, arthralgia, fever, chills, skin rash, anorexia, nausea, vomiting), timing of collection, body temperature monitoring, local reactions, infusion reaction management. Use prefix SAF-SOL. For any multi-step reaction-management / emergency procedure, emit ONE KRI PER action (drug+dose, sequence step, observation period). Output only NEW KRIs."),
        ("DILI thresholds",
        ...
        ("Concomitant & prohibited therapies",
         "Read the Concomitant / Prohibited Therapy section (often Section 6 or 7). Extract every DURING-study restriction on prohibited or conditionally-permitted therapies — one KRI per named agent/drug class with its exact condition. Use prefix SAF-CONMED. For any during-study washout / drug-free-interval requirement, encode EVERY facet stated (the interval, any half-life-multiple extension for long-acting / extended-release forms, and any required abstinence during the reporting period) — not just the headline. Include conditional permissions verbatim (e.g. 'permitted only if the dose is stable for at least 2 months prior to Day 0', 'allowed up to X mg/day for a non-study indication') — do not skip an item because it is phrased as a permission rather than a prohibition. Capture ONLY the during-study half; the pre-study lookback exclusion for the same agent is captured in the ELIG 'Exclusion criteria' turn — they are paired, not duplicates. Output only NEW KRIs."),
    ],
    "END": [
