...
15. **`rule_for_llm` must be binary and machine-readable**: The `rule_for_llm` field must be binary, unambiguous, and machine-readable — written as a precise, checkable instruction for an LLM system. It must specify exactly WHAT to check, WHEN to check it, HOW to verify compliance, and in relation to WHAT protocol requirement. It must produce a clear YES/NO answer when applied to a data record. It must NOT be narrative prose, a paraphrase of the protocol, or a vague description of intent. Examples of violations: "Ensure the procedure was performed correctly" (not binary), "Check the endpoint definition" (not specific), "The study drug was administered" (not an instruction). When the protocol uses qualitative language (e.g., "in the investigator's opinion", "as soon as possible"), write the rule_for_llm as faithfully as the protocol allows — preserve the qualitative wording verbatim rather than inventing a quantitative threshold the protocol does not state. Downstream filtering of qualitative / non-binary rules happens outside the boundaries of this skill. **Every protocol-stated conditional that the rule depends on (e.g. footnote-bound "only if", "only when", "unless") must appear inside `rule_for_llm` itself — typically as a precondition in CHECK. Conditionals stated only in `combined_ref` or `supporting_quote` are cited but not enforced; the downstream LLM evaluates `rule_for_llm` only.**

16. **Inference paths from adjacent tables**: Before treating a missing standalone table as `data_gap`, check whether the same answer is inferable from adjacent tables. Common patterns: eligibility-pass inferable from absence in a screen-failure log + presence of a randomization date; structure-still-present inferable from procedure-history tables (amputations, resections) before the visit date; procedure-occurred inferable from a boolean flag on the visit's row. When an inference path exists, name it in SOURCE and write CHECK as the inference (e.g. `IF subject not in screen_failures AND randomization_date is non-null: eligibility considered passed`). Only fall through to `data_gap` when no inference path exists.

17. **Timeliness windows on "is recorded" rules (SAF)**: Every SAF rule that depends on AE/SAE filing must explicitly state the protocol's entry window (e.g. "within 24h of investigator awareness", "within 72h of the AE-discussion visit") in CHECK. A rule that asks "is the AE recorded" with no time window satisfies itself whenever the AE is on file at any point — even years later — and silently misses late filings. If the awareness-time data isn't captured in EDC, name the best-available proxy (e.g. `date_of_entry` minus `onset_date`) in SOURCE and document the limitation in the rule body. Cross-reference: extends Quality Rule 1 (Faithfulness).

18. **Process / chain-of-custody rules without EDC evidence — out of scope**: Some protocol obligations describe process steps (transcription chains, audit trails, paper prescription QA, on-site personnel identity) whose evidence lives outside EDC (paper forms, pharmacy QA logs, monitor signoffs). When the cited evidence is not capturable in any EDC table for any subject, **do not author the KRI**, even if Quality Rule 14 would otherwise classify the cited sentence as "definitional". Document in the discard record as `"chain-of-custody / process — not in EDC"`. **This rule takes precedence over Rule 14 for the narrow case of process/SOP definitions whose verification artifact is exclusively non-EDC.**

19. **Investigator-judgment conditionals (Footnote-4 pattern)**: When the protocol gates a requirement on investigator clinical judgment ("only if deemed necessary", "in the investigator's opinion"), the judgment itself is typically NOT captured in EDC. Two valid authoring choices: (a) bind the rule to a proxy gate that IS in EDC (e.g. "if an AE was filed at or near this visit, then the assessment was likely needed") and encode the proxy in CHECK; OR (b) flag the rule as untestable at extraction time and skip it. Do NOT write the rule as if the requirement is absolute — it produces noise on every subject for whom the judgment conditional didn't trigger.

20. **Subset-rule check during authoring**: Before adding a narrower rule (e.g. an SAE-only 24h-reporting rule), search the in-progress set for a strict superset (e.g. "SAE OR Grade 3+ OR withdrawal — 24h"). If the narrower rule adds no new detection capability and no severity attribution, skip it; the broader rule already covers it. If both must exist (e.g. for severity routing), annotate the relationship in `kri_name` or `description`. This rule operates at authoring time and complements the existing dedup pass (Step 4A-Dedup) — dedup catches identical-rule duplicates, this rule catches narrower-overlaps-broader cases that dedup would otherwise keep.

21. **Severity calibration — clinical vs data-quality omissions**: Distinguish two failure modes in DEVIATION wording. (1) Clinical assessment omission (whole record missing, procedure not performed) → assign protocol-driven severity (often `major`). (2) Data-quality omission (record present but date null, units inconsistent across visits, sub-field skipped while the main field is captured) → typically `minor` — the procedure happened, the EDC entry was sloppy. When the rule could trigger on both modes, prefer two KRIs (one per severity) over a mixed-severity rule. Per-visit lab and per-visit assessment rules are the most common offenders.

22. **Name the source table explicitly in SOURCE**: SOURCE must reference exact source table names (e.g. `edc_adverse_events`, `raw_vital`, `micro_culture_results`), not narrative paraphrases ("the vital-signs record"). If multiple candidate tables could satisfy the rule, list all candidates in SOURCE. Exact names make downstream wiring verification possible and prevent the downstream LLM from guessing which table to read.
