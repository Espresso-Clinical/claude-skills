...
Six checks per judge (C6 added per SKILL.md update — closes the line-1163 enforcement
gap that previously let compound non-atomic KRIs pass; C3 extended for SOA footnoted KRIs):
  ...
  C3 Reference Accuracy   — cited page is ABOUT the clinical topic (semantic check);
                            for SOA footnoted KRIs, also verifies footnote-number/quote
                            alignment (quote text must come from the cited Footnote N,
                            not Footnote M)
  ...
  C6 Atomicity            — KRI encodes exactly ONE binary obligation about ONE
                            procedure (or procedure-alias) at ONE visit (or visit-alias)
                            with at most one condition; compound KRIs FAIL C6 and are
                            auto-split into N atomic KRIs and re-judged
...
Auto-correction:
  ...
  compound KRI into N atomic KRIs (per Step 1D-i / 1D-ii rules), bind each to its
  topic-specific footnote fragment per Step 1D-iv, then re-run the panel on each
  split KRI. Original compound KRI does NOT pass.
...
- C3 Reference Accuracy: The cited section + page is ABOUT the clinical topic of this KRI. This is a SEMANTIC check — not a substring match of the quote. If the KRI is about "LDL-C percent change at Week 14" but the cited page is about infusion reactions, C3 FAILS even if the quote text happens to appear on that page. ADDITIONALLY for SOA KRIs that cite a footnote (e.g., reference says "Footnote 12"): the supporting_quote MUST come from Footnote 12's text. If the quote is text from Footnote 13 or any other footnote, C3 FAILS even if the page is correct.
