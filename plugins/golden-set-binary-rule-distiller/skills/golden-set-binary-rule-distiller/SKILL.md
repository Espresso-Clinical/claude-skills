...
**Discretionary / optional procedures — three outcomes, not an automatic drop.** A procedure left to discretion — "may be performed", "optionally", "in a subset", "per Sponsor instruction", "per investigator judgment" — is **NOT** kept as an ordinary presence rule (non-performance can't be a deviation, so "if it is performed, its presence is checkable" doesn't justify one), but it is **not** an automatic drop either. Choose: **(1) Invert and keep** when the authorization is *documentable* (a Sponsor instruction, an imaging-plan designation, a recorded per-protocol gate) — the deviation is performing it *without* documented authorization, omission is explicitly not a deviation (author per `references/rewrite_format.md` rule 11). **(2) Keep as a scoped presence rule** when the protocol **requires** it for a protocol-**defined, recorded** subset or trigger (a defined recorded condition/denominator — e.g. "if total bilirubin is abnormal", "women of childbearing potential", "subjects at applicable sites" — AND the action is required when it holds). **(3) Confirm-drop** only when there is neither a documentable authorization to check against nor a protocol-defined recorded trigger — pure unanchored discretion.
...
- **Stage 2 (per family):** drop on a clear confirm-majority (≥ 4/5, or ≥⅔ pooled); restore if defend ≥ 3/5 or confirm ≤ 1; **escalate every 3–2 near-tie**. Equivalent rules (same footnote / procedure / "optional" clause) get ONE pooled verdict. Discretionary/optional procedures: invert-and-keep if authorization is documentable, keep-scoped if required for a defined recorded subset, else confirm-drop (see Stage 2).
...
- **Metadata hygiene is flagged, not fixed — it's the extractor's job.** Sentence-fragment Names, authoring meta-notes in Descriptions, and malformed/orphan provenance are surfaced in the audit log; the distiller never edits those columns (Constraint #1). Fixing them lives upstream in the extractor that produces them.
...
- **Discretionary ≠ automatic drop — invert it.** "May / optionally / in a subset / per Sponsor instruction" is not kept as a presence rule ("if performed, presence is checkable" isn't enough — non-performance isn't a deviation). But when the authorization is *documentable*, **invert**: the deviation is *performing it without documented authorization* (omission is not a deviation). Keep as a scoped presence rule when required for a protocol-defined recorded subset/trigger; drop only pure unanchored discretion.
