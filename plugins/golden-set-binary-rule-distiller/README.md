...
2. **Stage 2 — Drop defense** — a Gemini panel tries to *keep* each candidate. Equivalent rules (same footnote / procedure / "optional" clause) are clustered and ruled as one family; a family is dropped on a clear ≥ 4/5 confirm, restored if defended ≥ 3/5, and **every 3–2 near-tie is escalated to the user**. Discretionary/optional procedures ("may", "per Sponsor") are not kept as ordinary presence rules, but they are invert-kept when the authorization is documentable (deviation = performed *without* authorization; omission is not a deviation), kept-scoped when required for a defined recorded subset, and confirm-dropped only for pure unanchored discretion. An inverse-coverage pass then checks the whole drop list for systematic over-dropping.
